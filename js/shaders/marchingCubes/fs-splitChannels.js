...
void main(void) {
    ...
    vec2 uv  = (pos3D.xz + u3D_l.y * vec2(mod(pos3D.y, u3D_l.z), floor(pos3D.y / u3D_l.z)) + vec2(0.5)) / u3D_l.x;
    uv.y = fract(uv.y);
    
    ivec4 data = ivec4(texture(uDataTexture, uv));
    vec4 d1 = vec4(intToRGBA(data.r)) / 255.;
    vec4 d2 = vec4(intToRGBA(data.g)) / 255.;
    vec4 d3 = vec4(intToRGBA(data.b)) / 255.;
    vec4 d4 = vec4(intToRGBA(data.a)) / 255.;
